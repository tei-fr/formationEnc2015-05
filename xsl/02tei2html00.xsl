...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" version="2.0"
      ...
      <body>
        ...
        <div id="droite">
          <div id="tdm">
            <ul>
              <li>
                <a href="#metadonnees-textuelles"
                  >Informations sur le document</a>
              </li>
              <li>
          ...
          <div id="body">
            ...
            <xsl:if test="$commentaires = 'oui'">
              <div id="commentaires">
                ...
                <h2>Commentaires</h2>
                <xsl:apply-templates
                  select="text/body/div[@type = 'commentaires']"/>
                <p>
                  <a href="#tdm">Retour au début</a>
                </p>
              </div>
  ...
  <xsl:template match="dimensions">
    <xsl:value-of
      select="concat('larg. ', width, ' x haut. ', height, ' ', @unit)"/>
  </xsl:template>
  ...
  <xsl:template
    match="div[@type = 'transcription']//placeName | summary//placeName">
      ...
      <xsl:when test="@type = 'note'">
        ...
        <sup>
          <a href="{@target}">
            <xsl:attribute name="title">
              <xsl:value-of
                select="normalize-space(ancestor::text/back//div[@type = 'notes']/note[@xml:id = $monRef])"
              />
            </xsl:attribute>
  ...
  <xsl:template match="location">
    ...
    <xsl:if test="geo">
      ...
      <span class="geo">
        ...
        <xsl:value-of
          select="concat('lat. ', substring-before(geo, ' '), ', long. ', substring-after(geo, ' '))"
        />
      </span>
    </xsl:if>
  </xsl:template>
